...
function Select(props) {
  ...
  const closeSelect = (Index) => {
    ...
    if (item.textContent.length >= 17) {
      var css = document.styleSheets[3].cssRules[2].style;
     ...
     {props.placeholder==="Your Location" && <LocationOnOutlinedIcon style={{opacity:'0.5 '}}/>}
        ...
        {isOpenSelect && (
          <div className="selectDrop">
            ...
            <li key={0} onClick={()=>closeSelect(0)} className={`${selectedIndex === 0 ? "active" : ""}`}>{props.placeholder}</li>
            {
              props && props.data.map((category,Index)=>{
              return(
                <li key={Index+1} onClick={()=>closeSelect(Index+1)} className={`${selectedIndex === Index+1 ? "active" : ""}`}>{category}</li>
              )
            })
            }
              {/* <li
